...
public class PluginInterface : OptionInterface
{
  ...
  public override void Initialize()
  {
    ...
    bodyColorOptions = [
      ...
      new OpComboBox(bodyColorMode, new Vector2(170f, 450f), 130f, [.. OpResourceSelector.GetEnumNames(null, typeof(BodyColorMode))]),
    ];
    ...
    eyeColorOptions = [
      ...
      new OpComboBox(eyeColorMode, new Vector2(470f, 150f), 130f, [.. OpResourceSelector.GetEnumNames(null, typeof(EyeColorMode))]),
    ];
    ...
    (options[1] as OpCheckBox).OnValueChanged += (config, value, oldValue) => _use_body_color = value == "true";
    (options[3] as OpCheckBox).OnValueChanged += (config, value, oldValue) => _use_eye_color = value == "true";
    (options[5] as OpCheckBox).OnValueChanged += (config, value, oldValue) => _use_nickname = value == "true";
    (nicknameOptions[1] as OpCheckBox).OnValueChanged += (config, value, oldValue) => _show_arena_stats = value == "true";
    (nicknameOptions[5] as OpCheckBox).OnValueChanged += (config, value, oldValue) => _use_custom_name = value == "true";
    (bodyColorOptions[3] as OpComboBox).OnValueChanged += (config, value, oldValue) =>
    _body_color_mode = (BodyColorMode)Enum.Parse(typeof(BodyColorMode), value);
    (eyeColorOptions[11] as OpComboBox).OnValueChanged += (config, value, oldValue) =>
