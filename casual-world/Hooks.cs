...
public class Hooks(PluginInterface pluginInterface)
{
  public class RandomFriendTracker(FriendTracker ft)
  {
    public static int tickLimit = 40 * 60;

    public FriendTracker ft = ft;
    public int tick = 0;
  }

  public static CreatureTemplate.Relationship staticFriendRelationship = new(CreatureTemplate.Relationship.Type.Ignores, 1f);

  public PluginInterface pluginInterface = pluginInterface;
  public Dictionary<EntityID, RandomFriendTracker> randomFriendTracker = [];
  ...
  public void Spider_Attached(On.Spider.orig_Attached orig, Spider self)
  {
    BodyChunk bodyChunk = self.grasps[0].grabbed.bodyChunks[self.grasps[0].chunkGrabbed];
    if (IgnoreCondition(bodyChunk.owner.abstractPhysicalObject as AbstractCreature))
      orig(self);
  }
  ...
  public void Creature_Update(On.Creature.orig_Update orig, Creature self, bool eu)
  {
    ...
    if (RandomFriends)
    {
      if (!randomFriendTracker.TryGetValue(self.abstractCreature.ID, out RandomFriendTracker creatureFriend)
